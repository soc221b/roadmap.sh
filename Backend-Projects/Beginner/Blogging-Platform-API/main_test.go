	...
	var tests = []test{
		{
			...
			expectSql: func(DBMock sqlmock.Sqlmock) {
				DBMock.ExpectExec("^INSERT INTO posts \\(title, content, category, tags\\) VALUES \\(\\?, \\?, \\?, \\?\\)$").WithArgs("My First Blog Post", "This is the content of my first blog post.", "Technology", "Tech Programming").WillReturnResult(sqlmock.NewResult(1, 1))
